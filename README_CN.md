...
- **剪辑工程**：将当前素材、剪辑区间、时间轴视图和跨素材导出队列保存为 `.tscut` 文件，之后可继续编辑；仍在录制的 TS 文件新增内容后也可恢复工程。
- **灵活输出**：可以立即保存单个剪辑、加入批量导出队列，或将多个选中剪辑合并为一个 TS 文件。
- **无损媒体复制**：保留原始音视频编码，不进行转码。
- **截图与媒体信息**：保存或复制当前预览帧，并查看已打开文件的流信息。
- **高清缩略图图集**：按可配置的 N×M 网格生成最高 12000 像素宽的图集，支持缩放、拖动预览并导出 PNG 或 JPEG；可附带时间码及分类的视频、音频和字幕信息，包括语言以及 HDR、HLG 或杜比视界标识。
- **通用视频预览**：支持打开 MP4、MKV、MOV、WebM 及其他 FFmpeg 支持的视频格式进行预览、定位、截图和查看媒体信息。非 TS 文件暂只支持预览。
...
5. 需要稍后继续时，从“文件”菜单保存工程；之后打开 `.tscut` 工程即可恢复剪辑和导出队列。仍在增长的源文件可继续使用；如果已保存部分的校验不通过，则会拒绝恢复。
