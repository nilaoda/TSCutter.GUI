...
- 同一来源被多次录制，TS 包顺序和内容一致，但连续计数器不同，需要去除重叠后再拼接。

重叠判定基于 TS 包，不根据画面或音频听感猜测。
...
- 按当前顺序直接二进制追加；
- 查找相邻文件的后缀与前缀重叠；
- 忽略连续计数器差异，逐包验证同源录制的重叠，并在输出接缝处接续计数器；
- 在 TS 包边界上定位并完整验证重叠；
- 跳过被前一有效文件完整包含的文件；
- 使用有界内存流式生成输出；
- 在分析和输出期间检测源文件变化。
...
- 不匹配音视频内容相同、但 TS 包排列或负载不同的录制；
- 不修复源文件内部已有的连续计数器错误、PCR、PTS、DTS 或 PES；
- 不重新生成 PAT、PMT、SDT 等节目表；
- 不调整输入文件内部已有的时间轴；
- 不猜测没有可靠 TS 包证据的接缝位置。

如果文件经过重新复用，导致 TS 包排列或负载不同，当前模式不会自动裁剪接缝。
...
此模式在后一文件开头稍后的位置取 TS 包特征，在前一文件尾部寻找对应包，并从候选位置逐包验证到前一文件结尾。比较时仅忽略每个包的连续计数器；节目数据、包顺序和其他字段仍须一致。后一文件开头由录制程序添加的少量不同包可以被跳过。

输出只追加后一文件中对应于前一文件结尾之后的包，并调整新文件的连续计数器，使每个 PID 在接缝处接续。若逐包验证失败，该接缝会保持未匹配状态，不自动裁剪。
...
- 批量选择文件，也可拖入文件或文件夹；
- 拖入文件夹时只导入该目录第一层的 TS 文件，不递归扫描子目录；
- 多选移除；
- 上移和下移；
- 文件名自然排序；
- 明确的顺序编号。
...
严格模式要求前一文件后缀与后一文件前缀逐字节一致。同源模式允许后一文件有不同的起始封包，但从匹配锚点到前一文件末尾的包内容和顺序必须一致，连续计数器除外。
...
分析先使用具有区分度的包特征快速缩小候选范围，再对可能的重叠区域逐字节或逐包比较。
...
三种方式共享同一文件列表、顺序、输出路径和进度区域，因此在同一窗口内使用互斥模式切换。
...
这些结果描述的是 TS 包关系，不代表输出已经通过解码或时间轴检查。需要判断媒体质量时，应继续使用 TS 快速检查或播放器验证。

## 12. 已知限制

- 只支持 188 字节包结构；
- 只能分析列表中相邻有效文件的接缝；
- 同源模式只允许连续计数器不同，不识别重新复用后包排列或负载改变的重叠；
- 搜索范围不足时可能找不到真实重叠；
...
- 直接模式保持所有输入字节和顺序；
- 可靠后缀与前缀重叠只保留一份；
- 同源录制中仅连续计数器不同的重叠能够合并，接缝计数器保持连续；
- 未匹配接缝默认阻止输出；
- 明确允许时未匹配文件完整追加；
- 完整包含的文件不影响下一接缝；
- 大量小分片能够流式合并；
