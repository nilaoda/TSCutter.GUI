...
用于快速候选判断的包速率基线来自近期多个正常 PCR 间隔的中位数，而不是持续受最新一个
样本影响的平滑值。异常样本不会回灌基线，避免单个复用调度离群点污染后续判断。

明显的大幅时钟跳变可以立即标记为进一步分析候选；较小的速率离群则需要连续多次出现才会
提示。该分级规则用于降低复用抖动、分片边界和偶发 PCR 调度变化带来的误报，同时保留对
真实持续偏移的敏感度。

### 8.1 时间轴修复候选

快速检查只维护轻量的 PCR 间隔状态，用于判断是否存在值得进一步分析的时间轴偏差，
不会为普通扫描保存完整 PCR 样本或直接生成修复方案。

界面只有在确认存在可进一步分析的 PCR 间隔偏差时，才提供时间轴修复入口。进入独立修复
工具后仍需重新顺序读取文件，收集完整 PCR 样本并验证异常能否安全校正。快速检查中的
候选提示不等于最终一定可以修复。

以下情况需要明确区分：

- TEI、连续计数器缺口和 PES 错误是真实传输异常，不能被 PCR 校正掩盖；
- discontinuity 可能使图表分段或采用估算横轴，但不必然表示存在可修复问题；
- 文件开头正常的 PTS/PCR 固定偏移不属于 PCR 停滞或跳变；
- 只有检测到可修复候选时才允许直接打开修复窗口，避免出现“详情为空但提示 PCR 有问题”。
...
1. 定期取得同一节目中最近的音频和视频 PTS 偏移；
2. 使用启动阶段少量偏移样本的中位数建立固定基线；
3. 比较后续偏移与该基线的差值；
4. 只有漂移持续多次出现时才报告。

这样既能忽略合法固定延迟，也能过滤文件从半个 GOP 开始时首组音视频 PTS 并非同一播放
时刻造成的启动离群，同时发现后续逐渐累积的不同步。
...
摘要中的逐 PID 码率和时间轴中的瞬时码率用途不同：

- 摘要码率先从一段连续、可靠的 PCR 区间估算 TS 总传输码率，再按完整扫描范围内各 PID
  的包占比分摊，适合比较轨道在复用流中的平均占用；
- 时间轴码率按时间桶统计，适合观察码率波动、空白区间和异常附近的变化；
- 二者都是 TS 包层面的估算，包含 TS/PES 封装开销，不等同于编码器声明或解码得到的 ES
  净码率；
- 没有足够长且连续的可靠 PCR 区间时，摘要不猜测码率，而是显示不可用。
...
图表内部使用展开后的 PCR 相邻间隔推进横轴，不使用可能按 PTS 起点截断的展示时间。
这样既能保留正常的起始 PTS/PCR 偏移，也能在真实倒退、跳变或显式 discontinuity 处正确分段。
...
目录识别、连续性、PES 长度、时间戳、音视频同步、广播时间、摘要码率、详细事件和时间轴
可以按用途选择启用。只需要节目目录时，不保存完整异常和时间轴，也不执行与目录无关的
PES 或码率检查。摘要码率启用后，逐包热路径只维护数值状态，不保存逐包样本。
...
- 有可靠连续 PCR 的流应显示合理的逐 PID 估算码率，无可靠 PCR 时不应伪造数值；
- 构造时间戳倒退和跳变后应正确分类；
- 长文件扫描时事件和时间轴内存应保持有界；
- 报告中的时间点应能在播放器中定位到异常附近；
- 正常的文件起始 PTS/PCR 偏移不应触发估算时间轴或修复候选；
- 单个 PCR 调度离群或分片边界不应触发时间轴修复候选；
- 文件从半个 GOP 开始时，启动阶段音视频 PTS 偏移不应误报为持续漂移；
- 存在真实 PCR 跳变时应显示修复入口，独立工具复核后给出明确修复结论；
- 深浅色和多语言界面应保持状态表达一致。
