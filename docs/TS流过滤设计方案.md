...
流过滤器的输出仍然是带有节目结构和时间信息的 TS 文件。如果需要去除 TS 与 PES 封装、
得到供码流分析或其他工具继续处理的单轨原始数据，应使用 TS ES 提取器。
...
准备阶段至少读取 20 MiB 数据后才允许因目录稳定而提前结束。这给低频出现的 SDT、节目名称
和提供商信息更多被观察到的机会，也为按 PID 视图提供较稳定的码率样本。探测同时设有最大
范围；若节目表出现周期过长或文件结构不完整，工具会使用已取得的目录结束探测，而不是退化
为全文件扫描。因此极晚出现的服务可能需要由已知节目号回退展示，也可能因缺少完整 PMT 而
不可提取。

如果 SDT 缺失，按服务模式仍可使用节目号生成回退名称。只出现在 SDT 或 PAT 中、但没有完整 PMT 和轨道数据的服务可以展示出来，但不能提取。

按 PID 目录还会显示各 PID 的估算码率。该值使用可靠 PCR 推导 TS 总传输码率，再按采样
范围内的包占比分摊；它包含封装开销，不等同于 ES 净码率。没有可靠 PCR 时显示不可用，
而不是根据文件大小或未知时长强行估算。
...
- 只扫描足以建立稳定目录的数据；
- 在最小探测范围内持续补充节目元数据和码率样本；
- 服务元数据解析按需开启；
- 普通快速检查或按 PID 过滤不必保存额外服务目录；
- 不缓存完整媒体或节目单时间线。

码率探测只增加固定数量的时钟与包计数状态，不保存逐包码率样本，也不使内存随探测长度
增长。按服务过滤不需要展示逐 PID 码率时，可关闭这部分分析。
...
- 有可靠 PCR 时按 PID 目录显示合理的估算码率，无可靠 PCR 时显示不可用；
- 开关业务信息后输出 PID 符合预期。
