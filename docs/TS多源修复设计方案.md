...
- 输出时将候选时间偏移换算回参考文件的原始时间线，不会仅因匹配归一化而重写整个参考流。

关闭开关后，不收集参考 PCR 样本，也不执行辅助源时间轴分析。参考源和每个有效辅助源
各自只进行一次主扫描，但存在 PCR/PTS 偏移的区间可能无法建立候选关系。

该开关只影响多源匹配过程。它不会修改输入文件，也不会掩盖 TEI、连续计数器、PES 或媒体
损坏。需要单独生成完整时间轴校正版文件时，应使用独立的 TS 时间轴修复工具。
...
“仅部分轨道可补齐”和“无法安全补齐”。找到候选不等于自动写入：用户仍需在大段缺口页
勾选具体区间，并通过轨道页决定最终保留和修复哪些轨道。
...
- 对用户选中的大段缺失，将辅助源同一时间范围内的可用多轨 TS 包按原有交织顺序写入，
