...
时间轴改写规则可以嵌入其他 TS 工具的单遍输出管线。调用方应先将辅助来源的时间戳映射到
参考源原始时钟，再根据该包在参考文件中的目标位置应用最终校正，避免同一偏移被重复计算。
...
多源修复可以在匹配阶段和输出阶段分别复用相同的时间轴分析原则：

- 独立时间轴修复会生成新的完整校正文件；
- “匹配时自动校正时间轴”只建立虚拟连续时间线，用于跨来源对齐，不修改输入文件；
- “输出时同步修复时间轴”把已确认方案应用到多源修复的同一遍写出中，无需另行生成中间文件；
- 辅助包先映射到参考源原始时钟，再与参考包一起应用输出时间线校正；
- 多源修复识别到大段内容缺失时，区间匹配仍保留原始 PTS 缺片时长，避免虚拟连续时间线
  将真正需要补回的内容压缩掉；
- 完整大段替换已经覆盖的时间轴断点不会再次施加同一校正，避免接缝处出现反向跳变；
- 用户关闭匹配时自动校正后，多源修复不再执行额外辅助源时间轴分析，输出同步修复选项也随之关闭；
- 无论是否启用时间轴校正，独立的 TEI、连续计数器、PES 和媒体错误都不会被移除或隐藏。

因此，只处理单个文件时间线时应使用独立工具；执行多源媒体修复时，可以按需要分别决定
是否为匹配建立虚拟时间线，以及是否让最终输出同时获得校正后的参考时间线。
...
多源修复在启用匹配归一化时，会在参考源主扫描中同步收集所需 PCR，不额外完整读取参考源；
辅助源则需要额外的轻量时间轴分析。启用输出同步修复不会增加新的完整扫描，因为校正和复核
都在原本的输出遍历中完成。
...
- 多源输出中的辅助包只应用一次时间映射和最终校正；
- 被完整大段替换覆盖的时间轴断点不应在接缝处再次校正；
