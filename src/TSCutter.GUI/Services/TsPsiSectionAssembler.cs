...
internal delegate void TsPsiSectionHandler<TState>(Span<byte> section, ref TState state);
...
internal sealed class TsPsiSectionAssembler
{
    ...
    public void Push<TState>(
        ReadOnlySpan<byte> payload,
        bool payloadStart,
        ref TState state,
        TsPsiSectionHandler<TState> sectionHandler)
    {
        if (_waitForPayloadStart)
        {
            if (!payloadStart)
        ...
        if (payloadStart)
        {
            ...
            if (_length > 0 && pointer > 0)
                Append(payload[..pointer], ref state, sectionHandler);
            payload = payload[pointer..];
            Reset();
        }

        Append(payload, ref state, sectionHandler);
    }
        ...
        TsPsiSectionHandler<TState> sectionHandler)
    {
        while (!data.IsEmpty)
        {
            ...
            if (_expectedLength > 0 && _length == _expectedLength)
            {
                sectionHandler(_buffer.AsSpan(0, _length), ref state);
                Reset();
            }
        }
    }
